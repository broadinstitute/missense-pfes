...
| `results/enrichment_OR_by_protein_class.csv` | Odds ratios from Fisher's exact tests for all 103 protein features across 20 PANTHER protein functional classes, generated by `notebooks/compute_enrichment.ipynb`. Regenerated for v0.1.2 with the feature definitions shared with the scorer (lower-edge-inclusive bins; SS9 `-` excluded; see `glossary_protein_feature.md`) |
